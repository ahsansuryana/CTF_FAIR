...
router.post('/generate', requireAuth, requireParticipant, requireEventRunning, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (existingInstance) {
      const timeRemaining = Math.max(0, Math.floor((existingInstance.expiresAt.getTime() - Date.now()) / 1000));
      res.json({
        success: true,
        data: {
          instanceId: existingInstance.id,
          url: `http://${process.env.DOMAIN || 'localhost'}:${existingInstance.assignedPort}`,
          expiresAt: existingInstance.expiresAt,
          timeRemaining,
          port: existingInstance.assignedPort,
        },
      });
      return;
    }
