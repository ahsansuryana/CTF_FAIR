    ...
    return (
      <main className="min-h-screen bg-bg-base flex items-center justify-center">
        <div className="text-center max-w-md px-8">
          <div className="w-14 h-14 mx-auto mb-6 rounded-full bg-warning/15 flex items-center justify-center">
            <svg className="w-7 h-7 text-warning" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <h2 className="font-display text-2xl font-semibold text-text-primary mb-2">Event Ended</h2>
          <p className="text-text-secondary text-sm mb-8">Competition has ended. Challenges are no longer accessible.</p>
          <button
    ...
    <main className="min-h-screen bg-bg-base">
      <div className="max-w-content mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-display text-2xl font-semibold text-text-primary">{user?.username || 'Participant'}</h1>
            <p className="text-text-muted text-sm mt-0.5">Challenge Dashboard</p>
          </div>
            ...
            {myStats && (
              <>
                <div className="text-center">
                  ...
                  <p className="text-text-muted text-[10px] font-label uppercase tracking-wider">Rank</p>
                </div>
                <div className="w-px h-8 bg-border-subtle" />
                <div className="text-center">
                  <p className="font-mono text-lg font-semibold text-accent">{myStats.points}</p>
                  <p className="text-text-muted text-[10px] font-label uppercase tracking-wider">Points</p>
                </div>
                <div className="w-px h-8 bg-border-subtle" />
                <div className="text-center">
                  <p className="font-mono text-lg font-semibold text-accent">{myStats.solves}</p>
                  <p className="text-text-muted text-[10px] font-label uppercase tracking-wider">Solves</p>
                </div>
              </>
            )}
            <button
        ...
        {challenges.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-text-secondary text-sm">No challenges available yet</p>
            <p className="text-text-muted text-xs">Challenges will appear here once the event starts</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {challenges.map((challenge, index) => (
              ...
              >
                {challenge.isLocked ? (
                  <div className="bg-bg-surface border border-border-subtle rounded-card p-5 opacity-50 cursor-not-allowed">
                    <div className="flex items-center justify-between mb-3">
                      <span className={`text-[10px] font-label px-2 py-1 rounded-badge border ${categoryColors[challenge.category] || ''}`}>
                        {challenge.category.toUpperCase()}
                      </span>
                      <span className="font-mono text-sm text-accent">{challenge.points}pts</span>
                    </div>
                    <h3 className="font-display text-base font-medium text-text-disabled mb-2">{challenge.title}</h3>
                  ...
                  <Link to={`/challenge/${challenge.id}`} className="block">
                    <div
                      className={`bg-bg-surface border rounded-card p-5 transition-duration-micro hover:-translate-y-0.5 ${
                        challenge.isSolved
                    ...
                    >
                      ...
                      <h3 className="font-display text-base font-medium text-text-primary mb-2">{challenge.title}</h3>
                      <p className="text-text-secondary text-sm leading-relaxed line-clamp-2">{challenge.description}</p>
